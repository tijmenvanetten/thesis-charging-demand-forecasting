...
def train_predict(model, series_train, series_test, horizon, train_split=None, retrain=None):
...
def train_predict_past_covariates(model, series_train, series_test, past_covariates_train, past_covariates_test, horizon, train_split=None, retrain=False):
    ...
    if train_split:
        series_train, series_val = series_train.split_after(train_split)
        past_covariates_train, past_covariates_val = past_covariates_train.split_after(
            train_split)
...
def train_predict_global(model, series_train, series_test, horizon, train_split=None, retrain=False):
    ...
    if train_split:
        series_train_train, series_train_val = split_series_list(
            series_train, train_split)
...
def split_series_list(series_list, split):
    ...
    for series_train_single in series_list:
        series_train_single, series_val_single = series_train_single.split_after(
            split)
...
def train_predict_global_past_covariates(model, series_train, series_test, past_covariates_train, past_covariates_test, horizon, train_split=None, retrain=False):
    ...
    series = [concatenate([series_train_single, series_test_single])
              for series_train_single, series_test_single in zip(series_train, series_test)]
    past_covariates = [concatenate([past_covariates_train_single, past_covariates_test_single])
                       for past_covariates_train_single, past_covariates_test_single in zip(past_covariates_train, past_covariates_test)]

    if train_split:
        series_train, series_val = split_series_list(series_train, train_split)
        past_covariates_train, past_covariates_val = split_series_list(
            past_covariates_train, train_split)
